...
public class ValidationException(
    ...
    extendedRequestId: String? = null,
    cause: Throwable? = null,
) : EventBridgeException("ValidationException", message, statusCode, requestId, extendedRequestId, cause)
...
public class ThrottlingException(
    ...
    extendedRequestId: String? = null,
    cause: Throwable? = null,
) : EventBridgeException("ThrottlingException", message, statusCode, requestId, extendedRequestId, cause)
 ...
 * carries no request token and no rollback — see [EventBridge.putEvents]'s `NOT_IDEMPOTENT`
...
internal inline fun <T> mapErrors(block: () -> T): T = try {
    ...
    throw when (e.code) {
        ...
        else -> EventBridgeException(e.code, e.message, e.statusCode, e.requestId, e.extendedRequestId, e)
    }
}
