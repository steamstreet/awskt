...
## HTTP Engines
`aws-core`'s `awsHttpClient` runs on OkHttp on the JVM and on Curl on native. Keep the JVM engine one that pools connections. Through 3.1.1 it was CIO, which opens and closes a socket for every request and ran a consumer's test suite out of ephemeral ports. The JVM actual's KDoc says why OkHttp was chosen over the Java engine and over CIO with pipelining. It also covers the OkHttp defaults that awskt overrides: the per-host concurrency limit, the idle pool size and keep-alive, and `retryOnConnectionFailure`. Keep `retryOnConnectionFailure` off. With it on, OkHttp resends a write the server may already have applied. Turning it off exposed the stale keep-alive race, which failed production writes in 3.1.2 and 3.1.3. `StaleConnectionGuard` closes that race by discarding a stale pooled connection before the request is written. Its KDoc explains why a failure after the write must never be reclassified as retryable, even on a reused connection that received no response bytes. `AwsHttpClientJvmTest` checks connection reuse, those overrides, stale-connection handling and the timeouts against a real socket, so run `./gradlew :aws:aws-core:jvmTest` after any change to the engine or its configuration.
