...
public class ExpressionBuilder {
    ...
    private val nameReverseMap = mutableMapOf<String, String>()
    ...
    private fun getAttrName(attrName: String): String {
        val existing = nameReverseMap.get(attrName)
        if (existing != null) return existing

        val key = "#n${index++}"
        nameMap[key] = attrName
        nameReverseMap[attrName] = key
        return key
    }
    ...
    public inner class FilterAttribute(public val name: String) {
        public infix fun isOneOf(values: List<String>): Unit = valueIn(name, values)
