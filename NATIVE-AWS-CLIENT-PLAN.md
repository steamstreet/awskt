...
- [ ] **Transport-exception classification** (absent from the draft; this is the decision that determines whether `increment()` double-applies):
  ...
  - *Ambiguous* — socket read timeout, connection reset after write, incomplete response → retryable **only** for idempotent operations: GetItem, Query, Scan, BatchGetItem, TransactGetItems, DeleteItem, PutItem, TransactWriteItems *with a stable ClientRequestToken*. **NOT UpdateItem.** `MutableItem.kt:181-188` emits an unconditional numeric `ADD` and `:191-210` emits `list_append`; `dynamokt-exposed/.../TableOperations.kt:289` emits `ADD` too. All three double-apply on a retry after the bytes reached DynamoDB.
...
> per-operation retry safety (`UpdateItem` NOT_IDEMPOTENT, reads IDEMPOTENT), asserted; exit
...
| 7 | **Non-idempotent writes double-apply.** `increment()` emits `ADD`, `addToList()` emits `list_append`. | HIGH | Two-tier transport-exception classification; UpdateItem never retried on ambiguous failures; `retryAmbiguousWrites` escape hatch. |
